		...
		<div class="forms">
			<s:form action="leave_searchLeave.action" name="queryForm"
				...
				<s:if test='#session.employee_position == "员工"'>
					...
					<s:select name="leaveType" list="leaveTypeMap" listKey="key"
						listValue="value" headerKey="" headerValue="全部" theme="simple"></s:select>
				</s:if>
				<s:if test='#session.employee_position == "部门经理"'>
					<label>审批状态</label>
					<s:select name="leaveType" list="statusMap" listKey="key"
						listValue="value" headerKey="" headerValue="全部" theme="simple"></s:select>
				</s:if>
				...
				<tr>
					<td><a
						href="leave_getLeaveById.action?leave.id=<s:property value="#leave.id"/>"><s:property
								value="#leave.id" /></a></td>
					<td><s:property value="#leave.creator.name" />请假<s:property
					...
					<td><a
						href="leave_getLeaveById.action?leave.id=<s:property value="#leave.id"/>"><img
							src="${images}/search.gif" width="16" height="15" /></a> <s:if
							test="#leave.nextDeal.sn == #session.employee.sn">
							<s:if test='#session.employee_position == "部门经理" and #leave.status!="已审批"'>
								<a
									href="leave_toCheck.action?leave.id=<s:property value="#leave.id"/>">
